...
- `Addon.java` is the Meteor entrypoint (declared under `entrypoints.meteor` in `fabric.mod.json`). It registers the `DBK` category and every module. **New modules must be added to `Addon.onInitialize()`** and to the module lists in `README.md` and its Russian version `README.ru.md` (keep the two in sync).
...
- Meteor's Baritone jar is ProGuard-obfuscated (members renamed to `a`/`b`/`c`, private methods inlined), so don't mixin into Baritone internals. Changes to Baritone itself live in the fork at `../baritone` (branch `dbk-airplace`); DBK talks to fork-only features through `Settings.byLowerName` so it still builds against Meteor's Baritone (see `utils/BaritoneAirPlace`). `build.gradle.kts` always compiles against Meteor's Baritone, but if `../baritone/fabric/build/libs` has an unoptimized jar for our Minecraft version, `runClient` loads that instead (rebuild the fork with `./gradlew :fabric:build` there to pick up changes).
